...
func (m *Model) buildRestartTree() {
	...
	m.fetchRestartServiceStatus()
	m.applyRestartServiceStatusToTree()
}

func (m *Model) fetchRestartServiceStatus() {
	m.Restart.ServiceStatusMap = make(map[string]NodeStatus)
	if m.Config == nil {
		return
	}
	...
	for _, srv := range m.Config.Servers {
		...
		for _, proj := range projects {
			m.Restart.ServiceStatusMap[proj.Name] = StatusRunning
		}
